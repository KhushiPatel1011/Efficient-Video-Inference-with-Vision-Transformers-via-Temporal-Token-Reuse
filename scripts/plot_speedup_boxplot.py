...
Writes:
    results/plots/block_skip_speedup_boxplot.png
"""
...
def main():
    ...
    for t in THRESHOLDS:
        ...
        speedup_values = [
            to_float(r.get("speedup"))
            for r in skip_rows
            if to_float(r.get("speedup")) is not None
        ]

        if speedup_values:
            labels.append(f"{t:.2f}")
            speedup_data.append(speedup_values)
    ...
    plt.boxplot(speedup_data, labels=labels)
    plt.xlabel("CLS Threshold")
    plt.ylabel("Per-frame Speedup (baseline_ms / skip_ms)")
    plt.title("Speedup Distribution Across Frames Under Block-Skip Reuse")
    plt.tight_layout()

    out_path = PLOTS_DIR / "block_skip_speedup_boxplot.png"
